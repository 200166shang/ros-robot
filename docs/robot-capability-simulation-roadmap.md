...
> 更新说明：上文的“本轮没有修改”描述的是初稿只读调研；后续阶段 A、B 已实施，当前状态见第 8.2 节。
...
当前应用包含从原项目迁移的 9 个 ROS 包，并在阶段 B 新增 robot_head 仿真包，共 10 个应用包：`robot_bringup`、`robot_interfaces`、`robot_agent`、`robot_head`、`robot_voice`、`usb_camera`、`img_decode`、`rknn_yolov6`、`object_track`、`web_video_server`。
...
| B. 表情与虚拟头部 | 复用原表情资产；固定动作目录、ROS Action 仿真和网页状态；URDF/3D 关节后续再做 | 板端发布仿真状态，Mac 浏览器渲染 | “点头/微笑”触发可见动作，可取消 | 使用 sim 后端并返回仿真状态；不宣称真实舵机已验证 |
...
下列是目标边界；当前已新增 `robot_head`，其余拟议包按需求逐项实现，避免创建一堆空包。
...
按实际阶段选择完成时态：当前可以说已迁移原工程的九个 ROS 包，并新增了一个虚拟头部仿真包；已有固定音频驱动的软件链路记录。尚不能说已经实现 Nav2、真实底盘跟随或完整流式语音。
